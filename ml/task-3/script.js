...
function optimizeProfileSlide(slide) {
    const dualProfileContainer = slide.querySelector('.dual-profile-container');
    if (dualProfileContainer && isFullscreen) {
        dualProfileContainer.style.gridTemplateColumns = window.innerWidth < 1200 ? '1fr' : 'repeat(2, 1fr)';
        dualProfileContainer.style.gap = window.innerWidth < 768 ? '20px' : '30px';
        
        const profileCards = slide.querySelectorAll('.enhanced-profile-card');
        profileCards.forEach(card => {
            card.style.maxWidth = '100%';
            card.style.margin = '0 auto';
            
            const profileMain = card.querySelector('.profile-main');
            if (profileMain && window.innerWidth < 1024) {
                ...
                profileMain.style.gap = '25px';
            }
        });
        
        const teamInfoCard = slide.querySelector('.team-info-card');
        if (teamInfoCard) {
            teamInfoCard.style.maxWidth = '100%';
            teamInfoCard.style.margin = '0 auto';
        }
    }
}
...
function exportToPDF() {
    ...
    function createFinalPDF() {
        ...
        pdf.save('Review_Jurnal_ELM_Dewa_Ketut_Satriawan.pdf');
